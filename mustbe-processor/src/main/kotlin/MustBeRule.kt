...
enum class MustBeRule(
    ...
    DATA_CLASS(
        ...
        errorMessage = { name, declaration -> "Element '$name' must be a data class, but '${declaration.simpleName}' is not" }
    ),

    SEALED(
        annotation = MustBeSealed::class,
        requirement = { it.modifiers.contains(Modifier.SEALED) },
        errorMessage = { name, declaration -> "Element '$name' must be a sealed class, but '${declaration.simpleName}' is not" }
    ),

    ENUM(
        annotation = MustBeEnum::class,
        requirement = { it.modifiers.contains(Modifier.ENUM) },
        errorMessage = { name, declaration -> "Element '$name' must be an enum class, but '${declaration.simpleName}' is not" }
    ),

    INTERFACE(
        annotation = MustBeInterface::class,
        requirement = { it.classKind == ClassKind.INTERFACE },
        errorMessage = { name, declaration -> "Element '$name' must be an interface, but '${declaration.simpleName}' is not" }
    ),

    FINAL(
        annotation = MustBeFinal::class,
        requirement = { it.modifiers.contains(Modifier.FINAL) },
        errorMessage = { name, declaration -> "Element '$name' must be final, but '${declaration.simpleName}' is not" }
    ),

    OPEN(
        annotation = MustBeOpen::class,
        requirement = { it.modifiers.contains(Modifier.OPEN) },
        errorMessage = { name, declaration -> "Element '$name' must be open, but '${declaration.simpleName}' is not" }
    ),

    PUBLIC(
        annotation = MustBePublic::class,
        requirement = { it.modifiers.contains(Modifier.PUBLIC) },
        errorMessage = { name, declaration -> "Element '$name' must be public, but '${declaration.simpleName}' is not" }
    ),

    ABSTRACT(
        annotation = MustBeAbstract::class,
        requirement = { it.modifiers.contains(Modifier.ABSTRACT) },
        errorMessage = { name, declaration -> "Element '$name' must be abstract, but '${declaration.simpleName}' is not" }
    );
